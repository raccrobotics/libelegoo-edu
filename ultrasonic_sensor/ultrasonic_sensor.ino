...
DeviceDriverSet_UTLRASONIC Class

DeviceDriverSet_ULTRASONIC_Init() - Configuring the pin numbers for the Ultrasonic sensor, allows the motherboard on the robot to talk to the sensor
DeviceDriverSet_ULTRASONIC_Get() - Used to get the data from Ultrasonic sensor, all the necessary calculations are done in this function, it will return the distance of an obstacle that is in front of the Utlrasonic Sensor
...
void loop()
{
  ...
  /* Remeber x only contains the distance of an obstacle from the robot, we would then need to use motion control functions to move the robot around the obstacle */

  Serial.print("ULTRASONIC="); // Printing the distance (x) to the serial monitor - to access the serial monitor, select the magnifying glass icon on the top right of the IDE
  Serial.print(x);
  Serial.println("cm");
}
